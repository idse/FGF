...
if options.edgeDistance == false
    ...
    % or where the colony is much larger than the nominal radius
    % sometimes useful for visualizaiton
    s = 1.05;
    nuc_profile(radialgrid > nominalradius*s, :) = NaN;
    cyt_profile(radialgrid > nominalradius*s, :) = NaN;
    ratio_profile(radialgrid > nominalradius*s, :) = NaN;
    nuc_profile_colstd(radialgrid > nominalradius*s, :) = NaN;
    cyt_profile_colstd(radialgrid > nominalradius*s, :) = NaN;
    ratio_profile_colstd(radialgrid > nominalradius*s, :) = NaN;
end
...
if ~isempty(options.nucChannels)
    ...
    for cii = 1:numel(options.nucChannels)
        ...
        plot(r, nuc_profile(:,ci),'LineWidth',lw, 'Color', options.colors(plotcount,:))
        plotcount = plotcount+1;
        
        legendentries = [legendentries, [meta.channelLabel{ci}],[]];
    end
end
if ~isempty(options.cytChannels)
    
    plotcount2 = plotcount;
    for cii = numel(options.cytChannels):-1:1
        ci = options.cytChannels(cii) + 1;
        plot(r, cyt_profile(:,ci),'LineWidth',lw, 'Color', options.colors(plotcount2,:))
        plotcount2 = plotcount2+1;
...
if ~isempty(options.ratioChannels)
    ...
    plotcount3 = plotcount2;
    for cii = numel(options.ratioChannels):-1:1
        ci = options.ratioChannels(cii) + 1;

        plot(r, ratio_profile(:,ci),'LineWidth',3, 'Color', options.colors(plotcount3,:))
        plotcount3 = plotcount3+1;
...
if ~isempty(options.cytChannels)
    ...
    for cii = numel(options.cytChannels):-1:1
        ...
        if options.std
            if strcmp(options.stdMethod,'perColony')
                err = cyt_profile_colstd(:,ci);
            elseif strcmp(options.stdMethod,'neighborCells')
                err = cyt_profile_std(:,ci);
            end
            
            plerr = cyt_profile(:,ci) + err;
            minerr = cyt_profile(:,ci) - err;
            good = ~isnan(plerr);
            fill([r(good),fliplr(r(good))],[plerr(good)', fliplr(minerr(good)')],...
                options.colors(plotcount2,:),'FaceAlpha',0.2,'EdgeColor','none');
            plotcount2 = plotcount2+1;
        end
    end
    if log1p
...
if ~isempty(options.ratioChannels)
    ...
    for cii = numel(options.ratioChannels):-1:1
        ...
        if options.std
            if strcmp(options.stdMethod,'perColony')
                errorbar(r, ratio_profile(:,ci), ratio_profile_colstd(:,ci),'LineWidth',1, 'Color', options.colors(plotcount3,:),'HandleVisibility','off');
            elseif strcmp(options.stdMethod,'neighborCells')
                errorbar(r, ratio_profile(:,ci), ratio_profile_std(:,ci),'LineWidth',1, 'Color', options.colors(plotcount3,:),'HandleVisibility','off');
            end
            plotcount3 = plotcount3 + 1;
        end
    end
...
if options.legend
    legend(legendentries, 'Location','NorthEast','AutoUpdate','off'); % 'Location','best'
end
...
if options.edgeDistance
    xlabel('edge distance (\mum)', 'FontSize',fs,'FontWeight','Bold','Color',fgc)
else
    xlabel('r (\mum)', 'FontSize',fs,'FontWeight','Bold','Color',fgc)
end
