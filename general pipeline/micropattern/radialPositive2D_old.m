function result = radialPositive2D(positions, meta, channelThresholds, options)
    ...
    Redges = Rs(1:ptsperbinindividualcolonies:end);
    
    zptsperbinindividualcolonies = round(ptsperbinindividualcolonies*Rmax/Zmax);
    [Zs,Iz] = sort(Z);
    Zedges = Zs(1:zptsperbinindividualcolonies:end);
...
%     totaldensity = totalhist./(dZ*sqrt(dR'));
    ...
    neghist_norm(isnan(neghist_norm))=0;
    ...
    neghist = interp2(Rg,Zg,neghist,Rgl,Zgl);
    neghist(isnan(neghist)) = 0;
    
%     neghist_norm = interp2(Rg,Zg,neghist_norm,Rgl,Zgl);   
%     neghist_norm(isnan(neghist_norm)) = 0;
    
%     F = scatteredInterpolant(Rg(:),Zg(:),neghist_norm(:),'linear','none');
%     neghist_normnan = isnan(F(Rgl,Zgl));
    F = scatteredInterpolant(Rg(:),Zg(:),neghist_norm(:),'linear','nearest');
    neghist_norm = F(Rgl,Zgl);
    
    %anyposhist_norm = interp2(Rg,Zg,anyposhist_norm,Rgl,Zgl);   
    %anyposhist_norm(isnan(anyposhist_norm)) = 0;
    
%     F = scatteredInterpolant(Rg(:),Zg(:),anyposhist_norm(:),'linear','none');
%     anyposhist_normnan = isnan(F(Rgl,Zgl));
    F = scatteredInterpolant(Rg(:),Zg(:),anyposhist_norm(:),'linear','nearest');
    anyposhist_norm = F(Rgl,Zgl);
    
    % remember which values were NaN without extrapolation for later
%     neghistRnan = isnan(interp1(Rg(1,:),neghistR_norm,Rgl(1,:)));
%     neghistZnan = isnan(interp1(Zg(:,1),neghistZ_norm,Zgl(:,1)));
...
%     neghistR_norm(isnan(neghistR_norm)) = 0;
%     neghistZ_norm(isnan(neghistZ_norm)) = 0;
    
    for ci = 1:3
        poshist{ci} = interp2(Rg,Zg,poshist{ci},Rgl,Zgl);
        poshist{ci}(isnan(poshist{ci})) = 0; 
        
%         poshist_norm{ci} = interp2(Rg,Zg,poshist_norm{ci},Rgl,Zgl);
%         poshist_norm{ci}(isnan(poshist_norm{ci})) = 0; 

        F = scatteredInterpolant(Rg(:),Zg(:),poshist_norm{ci}(:),'linear','nearest');
        poshist_norm{ci} = F(Rgl,Zgl);

%         poshistR_norm{ci} = interp1(Rg(1,:),poshistR_norm{ci},Rgl(1,:));
%         poshistZ_norm{ci} = interp1(Zg(:,1),poshistZ_norm{ci},Zgl(:,1));
%         poshistRnan{ci} = isnan(interp1(Rg(1,:),poshistR_norm{ci},Rgl(1,:)));
%         poshistZnan{ci} = isnan(interp1(Zg(:,1),poshistZ_norm{ci},Zgl(:,1)));

        %poshistR_norm{ci}(isnan(poshistR_norm{ci})) = 0; 
        %poshistZ_norm{ci}(isnan(poshistZ_norm{ci})) = 0; 
