function micropatternPieVisConditions(dataDir, positions, options, meta)
    ...
    if ~isfield(options,'intensityScale')
        options.intensityScale = ones([1 numel(options.positionIdx)]);
    ...
    if ~isfield(options,'overlayweights')
        dapiweight = 0.8;
        imweight = 0.8;
    else
        imweight = options.overlayweights(1);
        dapiweight = options.overlayweights(2);
    end
    ...
    % RESTRICT POSITIONS TO SELECTED POSITIONS 
...
%         subDir = [prefix '_zslices'];
        ...
        elseif strcmp(positions(pi).filenameFormat(end-2:end),'nd2')
            
            prefix = positions(pi).filename(1:end-4);
            
            if ~isempty(dir(fullfile(dataDir,'filtered',[positions(pi).filename(1:end-4) '_filtered_MIP*'])))
                
                disp('using filtered MIP')
                subDir = 'filtered';
                fnameFormat = [positions(pi).filename(1:end-4) '_filtered_MIP_w%.4d.tif'];
                
            else
                %error('implement Nikon case without filtered stack');
                subDir = 'MIP';
                fnameFormat = [positions(pi).filename(1:end-4) '_MIP_w%.4d.tif'];
            end
        
        % Andor stitched file
        elseif strcmp(positions(pi).filenameFormat(end-2:end),'ims')
        ...
        for cii = 1:4
            ...
            try
                ...
                try
                    img = imread([fname(1:end-4) '.jpg']);
                catch
                    warning('problems with filename, default to positions.loadImage')
                    img = max(positions(pi).loadImage(d,cii-1,1),[],3);
                end
            end
            img = options.intensityScale(pi)*img;
            imgs{pi, cii} = img;
    ...
    DAPIim = combined{meta.nucChannel+1};
    if strcmp(options.color,'CMY')
        ...
        imwrite(imweight*overlay + dapiweight*DAPIim, fullfile(d, ['poscombined_' condstr '_' num2str(options.positionIdx) '_DAPI_CMY.png']));
    ...
    elseif strcmp(options.color,'RGB')
        ...
        imwrite(imweight*overlay + dapiweight*DAPIim, fullfile(d, ['poscombined_' condstr '_' num2str(options.positionIdx) '_DAPI_RGB.png']));
    ...
    elseif strcmp(options.color,'GM')
        ...
        imwrite(imweight*overlay + dapiweight*DAPIim, fullfile(d, ['poscombined_' condstr '_' num2str(options.positionIdx) '_DAPI_MG.png']));
    else
