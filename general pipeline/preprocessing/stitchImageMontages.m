...
%   because our images are always acquired as unsigned 16 bit integers (uint16)
...
if doFlatFieldCorrection
    %type of flat field correction can be 'full' or 'darkfieldsub'
    if isfield(opts,'flatFieldMode')
        flatFieldMode = opts.flatFieldMode;
    else
        flatFieldMode = 'full';
    end
end
...
listing = dir(fullfile(dataDir,['*',ext]));
if isempty(listing)
    %if stitching is run while acquisition is taking place & this function
    %is called before the first image file is transferred it will cause an
    %error
    error('no image files found')
end
r = bfGetReader(fullfile(dataDir,listing(1).name));
m = r.getSizeY; n = r.getSizeX;
...
%                     nt = r.getSizeT; %set nt above to allow us to set tmax more easily
                    ...
                    for ci = 1:nc 
                        ...
                        if strcmp(channelNames{ci},'DAPI (Em)')
                            channelNames{ci} = '488-DGRI';
                        end
                    end
                end
            ...
            for ci = 1:nc
                ...
                for zi = 1:nz
                    ...
                    if doFlatFieldCorrection
                        ...
                        elseif strcmp(flatFieldMode,'darkfieldsub')
                            if ci == 1 && zi == 1 && ti == 1
                                disp('subtracting dark field')
                            end
                            ims = flatFieldBGSub(ims,D);
                        end
                    end
