...
segDir = fullfile(dataDirs{r1},'MIP'); %segment images in the first directory
...
for pi = 1:npos
    ...
    segname = [prefix,'_FinalSegmentation.tif'];
    seg = imread(fullfile(segDir,segname)) == 1;
    mask = makeNucCytMasks(seg,[],opts);
    masks{pi} = mask;
...
parfor pi = 1:metas{r1}.nPositions
    ...
    for ri = 1:nrounds
        ...
        celldata = readoutFromMasks(imgs,masks{pi},opts);
        nucLevel = celldata.nucLevel; cytLevel = celldata.cytLevel;
        NCratio = celldata.NCratio; BG = celldata.background;
        ...
        if ri==1
            ...
            NCratios = NCratio;
            bgs = BG;
            positions(pi).cellData = celldata;
            positions(pi).ncells = size(celldata.XY,1);
        else
