...
minarea = 22/(meta.xres^2); %area threshold for getting rid of junk
maxarea = 273/(meta.xres^2); %area threshold for discarding fused clumps of nuclei
...
opts = struct(...
    'cytoSize',             5,... %width of the cytoplasmic annulus in pixels
    'cytoMargin',           2,... %margin from the nucleus to the cytoplasmic annulus
    'nucShrinkage',         2,... %number of pixels by which to shrink the nuclear mask (use 2 for 40x and 1 for 20x)
...
zopts = struct(...
    'IoU',                  0.35,... %intersection over union threshold for linking in z
    'maxZsize',             60,... %mazimum height for a nucleus in microns
    'minZsize',             3,... %minimum height for a nucleus in microns
    'maxCentroidDistance',  5,... %max distance between centroids of adjacent nuclei (not used if only IoU is true)
    'cytoMode',             '3D',... %'3D' or 'singleSlice'
    'onlyIoU', false,... %stitch only based on IoU, similar to cellpose's stitching algorithem.
    'min3DPixelsize', 0); %remove small objects after stitching, useful to remove junks. (35 is a good value for 20x)
...
%%% TODO - also handle for makeMIP when suffix doesn't have _t%.4d at the
%%% end %%%
...
parfor ii = 1:npos
    ...
    for zi = 1:nz
        ...
        finalseg = bwmorph(finalseg,'thicken',1);
