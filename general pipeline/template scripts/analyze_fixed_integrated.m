...
manMeta.channelLabel = {'DAPI','Smad23','SOX2','BRA'};

manMeta.conditions = {'0 48h mB','0 48h mB A50','0 48h mB C3',...
    '0 48h mB 2DG','0 48h mB 2DG A50','0 48h mB 2DG C3',...
    '0 24h mB','0 24h mB A50','0 24h mB C3',...
     '0 24h mB 2DG','0 24h mB 2DG A50','0 24h mB 2DG C3',...
    '24 48h mB','24 48h mB A50','24 48h mB C3',...
     '24 48h mB 2DG','24 48h mB 2DG A50','24 48h mB 2DG C3'};
 
%%
if strcmp(imageType,'MP')
    radii = [700 700 700 700 700 700 700 700 700 700 700 700 700 700 700 700 700 700]/2;
end

% % alternative definition in next block based on filelist
% posPerCondition = 3;
...
if JOB_Nikon == 1 || multiposition_Dragonfly == 1
    ...
    filelist = {};
    filelist{1} = dir(fullfile(dataDir,'*C01*Fusion*.ims'));
    filelist{2} = dir(fullfile(dataDir,'*C02*Fusion*.ims'));
    filelist{3} = dir(fullfile(dataDir,'*C03*Fusion*.ims'));
    filelist{4} = dir(fullfile(dataDir,'*C04*Fusion*.ims'));
    filelist{5} = dir(fullfile(dataDir,'*C05*Fusion*.ims'));
    filelist{6} = dir(fullfile(dataDir,'*C06*Fusion*.ims'));
    filelist{7} = dir(fullfile(dataDir,'*C07*Fusion*.ims'));
    filelist{8} = dir(fullfile(dataDir,'*C08*Fusion*.ims'));
    filelist{9} = dir(fullfile(dataDir,'*C09*Fusion*.ims'));
    filelist{10} = dir(fullfile(dataDir,'*C12*Fusion*.ims'));
    filelist{11} = dir(fullfile(dataDir,'*C11*Fusion*.ims'));
    filelist{12} = dir(fullfile(dataDir,'*C10*Fusion*.ims'));
    filelist{13} = dir(fullfile(dataDir,'*C13*Fusion*.ims'));
    filelist{14} = dir(fullfile(dataDir,'*C14*Fusion*.ims'));
    filelist{15} = dir(fullfile(dataDir,'*C15*Fusion*.ims'));
    filelist{16} = dir(fullfile(dataDir,'*C16*Fusion*.ims'));
    filelist{17} = dir(fullfile(dataDir,'*C17*Fusion*.ims'));
    filelist{18} = dir(fullfile(dataDir,'*C18*Fusion*.ims'));
end
...
tic
parpool(4)
parfor pi = 1:numel(positions)

    ci = find(pi >= conditionStartPos,1,'last'); % condition index
    cpi = pi - conditionStartPos(ci) + 1;

    fname = filelist{ci}(cpi).name;condi = find(pi >= conditionStartPos,1,'last');
...
options = struct('nucChannels', [2 3], ...'ratioMode', 'N:C',...
...
for condi = conditionindices
    ...
    figure()
    res{condi} = plotRadialProfiles(stats2, meta, options);
    %ylim([0 1.1]);
    %ylim([-0.0195 -0.016]);
    xlim([0 radii(condi)]);
    axis square
    title(meta.conditions{condi})
    saveas(gcf, ['radialProfile3_' meta.channelLabel{:} '_' meta.conditions{condi} '.png'])
    close;
end

%% radial intensity - conditions combined

colors = lines(numel(conditionindices)+1);
colors = colors([1 2 4:numel(conditionindices)+1],:);
...
for channel = markerChannels
    ...
    set(gca, 'LineWidth', 2);
    axis square;
    saveas(gcf, fullfile(dataDir, ['radialintensity_combined3_', meta.channelLabel{channel} '.png']));
end
...
for channel = markerChannels
    ...
    %legend(p,legendstr,'FontSize',15,'Location','best')
    legend(p,{'BMP','+2-DG','+2-DG+Act-A','+2DG+CHIR'},'FontSize',24,'Location','best')
    set(gca,'FontSize', fs)
    set(gca,'FontWeight', 'bold')
    set(gca, 'LineWidth', 2);
    axis square;
...
%channelThresholds = [0 650 220 200]; % manually adjust thresholds
channelThresholds = [0 650 220 85]; % manually adjust thresholds

for ci = 4% 1:numel(manMeta.channelLabel)
...
for condi = 1:numel(meta.conditions)
    combos =  {};%{[2 4],[2 3],[3 4]};
    stats.markerChannels = 4;
    [P,x,Pstd] = radialPositive(stats, positions, condi, meta, combos);
    Pall{condi} = P;
    Pstdall{condi} = Pstd;
    xall{condi} = x;
    ylim([0 1]);
    saveas(gcf, fullfile(dataDir, ['radialpositive_' meta.conditions{condi} '_', meta.channelLabel{stats.markerChannels} '.png']));
    close;
end

%% radial probability of being positive channel by channel (MP Only)

conditionindices = [13 16:18];%1:numel(meta.conditions);

%conditionindices = [7 10 11 12];
...
for condi = 15:numel(meta.conditions)

    close all;
    options = struct();

    options.selectDisType = [1, 3, 1,1]; % 1 - nuc, 2 - cyto, 3 - NCratio, 4 - CNratio
    options.channelThresholds = [0 1 220 85]; % re-define channelThres when plot cyto or NCratio
    %options.checkPos = badidxCon; % whether you want to check defined cells in scatter plot

    options.imageType = imageType;
    options.conditionIdx = condi;
    options.channelCombos =  {[3 2 4], [4 3 2], [4 2 3]};

    options.axisLabel = {'DAPI','Smad23','SOX2','BRA'};
    options.channelMax = exp([3 4 4 4])-1; %xlim or ylim
    options.log1p = [true true true true true true true true true true true true true true true true true true] ;
    if strcmp(imageType,'MP')
        options.radiusMicron = 350;
    end
...
options.tol = [0.01 0.99; 0.02 0.999; 0.01 0.99; 0.3 0.999];
...
posidx = [54 48 1]; % the first position sets the lookup table
...
conditionIdx = 13:18%numel(manMeta.conditions);
