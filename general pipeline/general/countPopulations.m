...
%                 intensitymean_combo(pi, ci, 2, 2, 2) = mean(log(1+(exp(1)-1)*nucLevel(nnnidx,ci)/stats.thresholds(ci)));%./nucLevel(nnnidx,1));
    ...
    for casei = cases
        ...
        elseif casei == 2 % NUMBERS OF EACH MARKER
            ...
            legendstr = meta.channelLabel(2:4);
            vals = (diag(NcellsTotal(conditionsidx))*positivefractionavg(conditionsidx,2:4)./meta.posPerCondition(conditionsidx))';
            errs = (diag(NcellsTotal(conditionsidx))*positivefractionstd(conditionsidx,2:4)./meta.posPerCondition(conditionsidx))';
        ...
        elseif casei == 3 || casei == 4 % COMBO=SUBSET FRACTION || NUMBERS
            ...
            if numel(combo) == 3
                ...
                for i = 1:4 % +-, --, ++, -+
                    ...
                    if casei == 4
                        dvals = NcellsTotal(conditionsidx)'.*dvals./meta.posPerCondition(conditionsidx);
                        derrs = NcellsTotal(conditionsidx)'.*derrs./meta.posPerCondition(conditionsidx);
                    elseif casei == 3
                        dvals = dvals*100;
                        derrs = derrs*100;
                    end
        ...
        elseif casei == 5 % CELL NUMBERS
            ...
            vals = NcellsTotal(conditionsidx)'./meta.posPerCondition(conditionsidx);
        ...
        if casei==5
            ...
            text(0.1,8.25,'x10^3','FontSize',fs);
        end
