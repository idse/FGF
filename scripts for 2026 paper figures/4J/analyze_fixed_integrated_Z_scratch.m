...
manMeta.channelLabel = {'DAPI','ISL1','TBXT','pERK'};
manMeta.conditions = {'30min No Scratch','30min Scratch','30min No Scratch FGFRi','30min Scratch FGFRi' ...
    '1h Scratch','3h Scratch','6h Scratch'};
...
options.tol = [0.1 0.995; 0.1 0.99; 0.1 0.995; 0.1 0.98];
...
posidx = [6 19 23 24 27]; % the first position sets the lookup table
...
for pi = [5:8,13:28]
    ...
    mask = ~bwareaopen(~mask,300); % fill small holes
    ...
    scratchmask(cat(1,CC.PixelIdxList{I(sortnumPixels > 200)})) = true;
    %imshow(scratchmask);
    ...
    % Create new mask with only the largest connected component
    scratchmask_largest = false(size(scratchmask));
    if ~isempty(maxIdx)
        scratchmask_largest(CC.PixelIdxList{maxIdx}) = true;
    end

    scratchmask = scratchmask_largest;  % replace original with largest only
...
conditionindices = [2,5,6,7,4];
colors = jet(numel(conditionindices));
colors = colors([1 2 3 5],:); % skip yellow and add red (easier to see)
...
for condi = conditionindices
    ...
    cutoff = min(sum(~isnan(x)),sum(~isnan(plerr)));
    good = 1:cutoff;
...
axis square;
legend({'30min','1h','3h','6h','+FGFRi'},'Location','NorthEast','FontSize',fs)

saveas(gcf, ['pERKprofile_scratch dynamics' '.png'])

%% plot time course of amplitude

x = [0 0.5 1 3 6];
baseline = (mean(meany{2}(end-10:end))-ymin)/(ymax-ymin);
y = [baseline];
yerr = std(meany{2}(end-10:end))/(ymax-ymin);

for condi = [2 5 6 7]
    
    [amplitude, idx] = nanmax(meany{condi});
    amplitude = (amplitude - ymin)/(ymax-ymin);
    amperr = stdy{condi}(idx)/(ymax-ymin);
    %amplitude = nanmean(meany{condi}(1:20));
    y = [y amplitude];
    yerr = [yerr amperr];
end

errorbar(x,y,yerr, linewidth=lw)
cleanSubplot(fs,lw);
%ylim([-0.1 1.1]);
yticks([0 1])
ylim([-0.1 1.1])
axis square;
box off 
ylabel('pERK peak level (a.u.) ');
xlabel('time (hr)');

saveas(gcf, ['pERKamplitude_scratch dynamics' '.png'])

%% Export pERK profile data to CSV
% Prepare data matrix with proper dimensions
x_col = x(:);  % Ensure column vector
exportData = x_col;  % First column is distance
headerNames = {'Distance_um'};

i = 0;
conditionNames = {'30min', '1h', '3h', '6h', 'FGFRi'};
for condi = conditionindices
    i = i+1;
    normmean = (meany{condi}-ymin)/(ymax-ymin);
    normstd = stdy{condi}/(ymax-ymin);
    
    % Ensure column vectors and matching length
    normmean = normmean(:);
    normstd = normstd(:);
    
    % Trim or pad to match x length if needed
    if length(normmean) > length(x_col)
        normmean = normmean(1:length(x_col));
        normstd = normstd(1:length(x_col));
    elseif length(normmean) < length(x_col)
        normmean = [normmean; nan(length(x_col)-length(normmean), 1)];
        normstd = [normstd; nan(length(x_col)-length(normstd), 1)];
    end
    
    exportData = [exportData, normmean, normstd];
    headerNames = [headerNames, ...
                   sprintf('%s_mean', conditionNames{i}), ...
                   sprintf('%s_std', conditionNames{i})];
end

% Write to CSV
T = array2table(exportData, 'VariableNames', headerNames);
writetable(T, 'pERK_profile_scratch_dynamics.csv');
disp('Exported pERK_profile_scratch_dynamics.csv');
%%
pi = 1;
options = struct();
options.outsideMargin = 50; % margin outside the disk shaped mask in pixels
