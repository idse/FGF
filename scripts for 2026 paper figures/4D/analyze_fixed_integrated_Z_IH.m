...
manMeta.channelLabel = {'DAPI','SOX2','NANOG','pERK'};
manMeta.conditions = {'-bFGF','bFGF 24h','bFGF 18h','bFGF 8h','bFGF 5h','bFGF 2h','bFGF 1h','bFGF 30min','bFGF 10min'};
manMeta.nChannels = numel(manMeta.channelLabel);
nWells = numel(manMeta.conditions);
...
radii = (700/2)*ones(numel(manMeta.conditions),1); 
...
for pi = pList %1:manMeta.nWells
    ...
    %nucimg = squeeze(img(:,:,1,:)); %readStack(fullfname);
    nucimg = readStack(fullfname);

    % load segmentation
    fullfname = char(sprintf(filenameFormat, pi-1, ci, 0));
    ...
    i = 1;
    xinds = 1:size(nucimg,2);
    ymarg = 10;
    yind = yinds{i}(1)-ymarg:yinds{i}(end)+ymarg;
    res = makeSegmentationCrossSection(nucimg, label, meta, yind, xinds);
...
options = struct('nucChannels',[1,2],'cytChannels', [3], ...'ratioMode', 'N:C',...
...
colors = parula(numel(conditionindices));
lw = 2;
fs = 26;
...
for channel = markerChannels
    ...
    for condi = conditionindices
        ...
        legendstr = [legendstr,{[meta.conditions{condi},' nuc ',meta.channelLabel{channel}]}];
    end
    i = 0;
    for condi = conditionindices
        i = i+1;
    ...
    legend(p,legendstr,'FontSize',10,'Location','best')
    set(gca,'FontSize', fs)
...
for channel = markerChannels
    ...
    for condi = conditionindices
        ...
        legendstr = [legendstr,{[meta.conditions{condi},' cyto ',meta.channelLabel{channel}]}];
    end
    i = 0;
    for condi = conditionindices
        i = i+1;
...
%% CYTOPLASMIC CHANNELS - MODIFIED HERE TO INCLUDE DEXTRA
colors = lines(numel(conditionindices));
lw = 2;
fs = 26;
parentDir = fileparts(dataDir);

detrandir = fullfile(parentDir, '20250327_mTminus DextranGFP_pERK in 20250401');
T = readtable(fullfile(detrandir, 'dextranprofile_24h.csv'));
conditionindices = 2;

extracolor = lines(3);
extracolor = extracolor(3,:);
markerChannels = options.cytChannels+1;
for channel = markerChannels
    figure()
    legendstr = [];
    ...
    plot(T.x, T.normmean,'LineWidth',lw,'Color',extracolor);
    legendstr = [legendstr, {'dextran'}];
    ...
    plerr = T.normmean + T.normstd;
    minerr = T.normmean - T.normstd;
    good = ~isnan(T.x');
    fill([T.x(good)',fliplr(T.x(good)')],[plerr(good)', fliplr(minerr(good)')],...
            extracolor,'FaceAlpha',0.2,'EdgeColor','none');
    ...
    legend(p,{'pERK','dextran'},'FontSize',20,'Location','NorthEast')
    set(gca,'FontSize', fs)
    set(gca,'FontWeight', 'bold')
    set(gca, 'LineWidth', 2);
    %title(meta.channelLabel{channel})
    axis square;
    saveas(gcf, fullfile(dataDir, ['radialintensity_combineDextran_', legendstr{:}, '_' meta.channelLabel{channel} '.png']));
end

%% CYTOPLASMIC CHANNELS - MODIFIED HERE TO INCLUDE DEXTRA NOT Norm
colors = lines(numel(conditionindices));
lw = 2;
fs = 26;
parentDir = fileparts(dataDir);
...
colors = lines(2);
channel = 4; % the pERK channel

meanERKProfile = res{condi}.cyt_profile(:,channel);
stdERKProfile  = res{condi}.cyt_profile_colstd(:,channel);

minERK = min(meanERKProfile);
maxERK = max(meanERKProfile);
rangeERK = maxERK - minERK;

meanERKProfile = (meanERKProfile - minERK) / rangeERK;
stdERKProfile  = stdERKProfile / rangeERK;

xERK = res{condi}.r;
ERKupper = meanERKProfile + stdERKProfile;
ERKlower = meanERKProfile - stdERKProfile;

meanDEXTRANProfile = T.normmean(T.x>0);
stdDEXTRANProfile  = T.normstd(T.x>0);
xDEXTRAN = T.x(T.x>0);

minDEXTRAN = min(meanDEXTRANProfile);
maxDEXTRAN = max(meanDEXTRANProfile);
rangeDEXTRAN = maxDEXTRAN - minDEXTRAN;

meanDEXTRANProfile = (meanDEXTRANProfile - minDEXTRAN) / rangeDEXTRAN;
stdDEXTRANProfile  = stdDEXTRANProfile / rangeDEXTRAN;

DEXTRANupper = meanDEXTRANProfile + stdDEXTRANProfile;
DEXTRANlower = meanDEXTRANProfile - stdDEXTRANProfile;
    
clf
hold on
legendstr = {};

plot(xERK', meanERKProfile,'LineWidth',lw,'Color',colors(1,:));
legendstr = [legendstr,meta.conditions(condi)];

plot(xDEXTRAN, meanDEXTRANProfile,'LineWidth',lw,'Color',colors(2,:));
legendstr = [legendstr, {'dextran'}];

good = ~isnan(ERKupper);    
fill([xERK(good)'; flipud(xERK(good)')], [ERKupper(good); flipud(ERKlower(good))], colors(1,:), ...
'FaceAlpha', 0.2, 'EdgeColor', 'none');

fill([xDEXTRAN',fliplr(xDEXTRAN')],[DEXTRANupper', fliplr(DEXTRANlower')],...
...
legend(p,{'pERK','dextran'},'FontSize',fs,'Location','NorthEast')
set(gca,'FontSize', fs)
set(gca,'FontWeight', 'bold')
set(gca, 'LineWidth', 2);
%title(meta.channelLabel{channel})
axis square;
saveas(gcf, fullfile(dataDir, ['radialintensity_combineDextran_NoNorm_', legendstr{:}, '_' meta.channelLabel{channel} '.png']));
...
cpi = 2; % position within condition index
s = 5;
...
channelThresholds = [0 250 100 150]; % manually adjust thresholds
...
options = struct('half',true,'color','CMY', 'channels',[2 3 4],'flip',false);
...
for condi = 2%:numel(meta.conditions)
    ...
    options.channelMax = exp([5 5 5 5])-1; %xlim or ylim
...
posidx = 5; % the first position sets the lookup table
